...
- O AI Content Classifier processa até 30 vídeos por hora e tem timeout de 55 minutos para não atravessar o próximo ciclo. Uma execução encerrada pelo timeout pode deixar o respectivo `pipeline_run` como `running` até a observabilidade marcá-lo como stale.
