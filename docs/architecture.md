...
O AI Content Classifier consulta até 30 linhas linguisticamente elegíveis de `select_classification_candidates` por execução horária, processa um vídeo por vez e conecta o Basic LLM Chain a um modelo NVIDIA e a um parser estruturado. A saída validada é persistida em colunas tipadas de `video_classifications`; falhas do modelo, do parser ou da persistência seguem rotas próprias para `pipeline_errors` e o loop continua. `video_classification_processing_state` impede retries imediatos: as duas primeiras falhas terminais recebem backoff exponencial e a terceira exige decisão manual. Um timeout de 55 minutos impede que o lote atravesse o próximo ciclo.
...
Todos os workflows produtivos registram seu ciclo de vida em `pipeline_runs` e falhas terminais sanitizadas em `pipeline_errors`. Runs interrompidos que excedem `OBSERVABILITY_STALE_RUN_MINUTES` podem ser reconciliados de `running` para `cancelled` por uma função transacional que preserva contadores e registra a inferência em `metadata`. O workflow `10 - TrendLens - Observability` consolida esses registros em uma janela fechada de 24 horas, calcula saúde, contadores, retries e duração por etapa e persiste snapshots JSON em `pipeline_observability_reports`.
