...
O Content Language Gate separa idioma declarado pela API, idioma-alvo e idioma detectado. Sinais explícitos da API recebem decisão determinística; registros sem idioma entram em uma fila com tentativas limitadas e backoff e são avaliados pelo modelo somente a partir de título e descrição. O vídeo continua preservado em `videos`, mas apenas `eligible` pode avançar ao classificador e às agregações analíticas. Uma regra central no PostgreSQL converte variantes portuguesas para `language=pt`, preserva a variante em `detected_language` e não altera `region=BR`.
...
O AI Content Classifier consulta até 30 linhas linguisticamente elegíveis de `select_classification_candidates` por execução horária e processa um vídeo por vez. Os três workflows de IA usam HTTP Request nativo contra o endpoint compatível com OpenAI da NVIDIA, com `response_format=json_object` e `chat_template_kwargs.enable_thinking=false`. Um Code node pequeno extrai e valida o objeto final porque o parser nativo não separava com segurança o raciocínio retornado pelo Nemotron. A saída validada é persistida em colunas tipadas de `video_classifications`; falhas HTTP, de contrato ou de persistência seguem rotas próprias para `pipeline_errors` e o loop continua.
...
O Recommendation AI chama `select_recommendation_candidates` e envia ao modelo somente evidências agregadas do bucket corrente cuja amostra alcança `MIN_SAMPLE_SIZE`. A política vale para a categoria principal, padrões categoria–formato–origem e os melhores formatos e hooks do contexto. A resposta JSON passa por validação de campos, tamanhos, unicidade e limites antes de ser persistida em `recommendations`; o hash da evidência e as versões de prompt, modelo e cálculos tornam o processamento idempotente e auditável.
...
Todos os workflows produtivos registram seu ciclo de vida em `pipeline_runs` e falhas terminais sanitizadas em `pipeline_errors`. O workflow `10 - TrendLens - Observability` reconcilia primeiro runs `running` obsoletos e só depois monta o relatório. A versão `v3-recovery-aware` mantém falhas históricas visíveis, mas reserva `critical` para execução obsoleta ou para falha ainda vigente no último run; um workflow que voltou a executar com sucesso passa a `degraded` enquanto o histórico permanecer na janela.
